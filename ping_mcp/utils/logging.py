"""
Unified logging configuration for PingOne MCP server.
...
def get_logger(name: str = None) -> logging.Logger:
    ...
    Args:
        name: Logger name (defaults to 'pingone_mcp' if None)
...
def configure_logging(log_level=None, console_level=None, suppress_mcp_logs=True):
    ...
    if suppress_mcp_logs:
        ...
        logging.getLogger("pingone_mcp").setLevel(logging.WARNING)
        logging.getLogger("pingone_mcp.utils").setLevel(logging.WARNING)
        logging.getLogger("pingone_mcp.tools").setLevel(logging.WARNING)
        logging.getLogger("pingone_mcp.tools.tool_registry").setLevel(logging.WARNING)
    
    return root_logger

def setup_protocol_logging(logger_name="pingone-mcp-server", fs_logger_name="filesystem", 
...
def format_json_with_newlines(data: Any) -> str:
    ...
    try:
        ...
        logger = logging.getLogger('pingone-mcp-server')
...
def extract_tool_info(data: dict) -> Optional[dict]:
    ...
    try:
        ...
        logging.getLogger("pingone-mcp-server").debug(f"Extracting from: {json.dumps(data, default=str)[:200]}")
        ...
        logging.getLogger("pingone-mcp-server").error(f"Error extracting tool info: {e}")
        return None
...
class LoggingMCPServerStdio:
    ...
    def __init__(self, python_path, script_args, env=None, protocol_logger=None, fs_logger=None):
        ...
        self.protocol_logger = protocol_logger or logging.getLogger("pingone-mcp-server")
    ...
    async def call_tool(self, name, parameters=None, **kwargs):
        ...
        self.protocol_logger.info(f"Directly calling tool: {name}")  # <-- THIS IS THE KEY LOG!
        if parameters:
